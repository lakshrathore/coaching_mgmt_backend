...
exports.getNotices = async (req, res) => {
  try {
    ...
    const params = [];
    if (role && role !== 'all') { query += ' AND (n.target_role = ? OR n.target_role = "all")'; params.push(role); }
    if (batch_id) { query += ' AND (n.batch_id = ? OR n.batch_id IS NULL)'; params.push(batch_id); }
    query += ' ORDER BY n.priority DESC, n.publish_date DESC LIMIT 50';
...
exports.createNotice = async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      `INSERT INTO notices (title, content, target_role, batch_id, priority, published_by, expiry_date)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [title, content, target_role || 'all', batch_id || null, priority || 'normal', req.user.id, expiry_date || null]
    );
...
exports.deleteNotice = async (req, res) => {
  try {
    await db.execute('UPDATE notices SET is_active = 0 WHERE id = ?', [req.params.id]);
...
exports.createHomework = async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      `INSERT INTO homework (title, description, batch_id, subject_id, due_date, assigned_by)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [title, description, batch_id, subject_id, due_date, req.user.id]
    );
...
exports.uploadMaterial = async (req, res) => {
  try {
    ...
    const file = req.file;
    const [result] = await db.execute(
      `INSERT INTO study_materials (title, description, batch_id, subject_id, file_path, file_type, file_size, is_public, uploaded_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [title, description, batch_id, subject_id,
       file ? `/uploads/${file.filename}` : null,
       file ? file.mimetype : null,
       file ? file.size : null,
       is_public ? 1 : 0, req.user.id]
    );
    res.status(201).json({ success: true, message: 'Material uploaded', id: result.insertId });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.createEnquiry = async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      `INSERT INTO enquiries (name, phone, email, interested_batch, source, notes, next_follow_up, assigned_to)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, phone, email, interested_batch, source || 'walk-in', notes, next_follow_up || null, assigned_to || null]
    );
    res.status(201).json({ success: true, message: 'Enquiry added', id: result.insertId });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.updateEnquiry = async (req, res) => {
  try {
    ...
    await db.execute(
      'UPDATE enquiries SET status=?, next_follow_up=?, notes=?, assigned_to=? WHERE id=?',
      [status, next_follow_up || null, notes, assigned_to || null, req.params.id]
    );
    if (notes) {
      await db.execute(
...
exports.addExpense = async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      `INSERT INTO expenses (title, amount, category, expense_date, payment_mode, notes, added_by)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [title, amount, category || 'other', expense_date || new Date().toISOString().split('T')[0],
       payment_mode || 'cash', notes, req.user.id]
    );
...
exports.getSubjects = async (req, res) => {
  try {
    const [rows] = await db.execute('SELECT * FROM subjects WHERE is_active = 1 ORDER BY name');
    res.json({ success: true, data: rows });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
exports.updateSettings = async (req, res) => {
  try {
    ...
    await db.execute(
      `UPDATE settings SET coaching_name=?, address=?, phone=?, email=?, currency=?, currency_symbol=?, academic_year=? WHERE id=1`,
      [coaching_name, address, phone, email, currency, currency_symbol, academic_year]
    );
...
exports.createFaculty = async (req, res) => {
  ...
  try {
    ...
    const [countRows] = await conn.execute('SELECT COUNT(*) as cnt FROM faculty');
    ...
    const [userResult] = await conn.execute(
      'INSERT INTO users (name, email, phone, password, role) VALUES (?, ?, ?, ?, "faculty")',
      [name, email || null, phone, hashed]
    );

    await conn.execute(
      `INSERT INTO faculty (user_id, employee_id, qualification, specialization, experience_years, joining_date, salary, address)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [userResult.insertId, employee_id, qualification, specialization, experience_years || 0, joining_date, salary, address]
    );
...
exports.getDashboardStats = async (req, res) => {
  try {
    const [[students]] = await db.execute(`SELECT COUNT(*) as total FROM users WHERE role='student' AND is_active=1`);
    const [[faculty]] = await db.execute(`SELECT COUNT(*) as total FROM users WHERE role='faculty' AND is_active=1`);
    const [[batches]] = await db.execute(`SELECT COUNT(*) as total FROM batches WHERE is_active=1`);
    const [[enquiries]] = await db.execute(`SELECT COUNT(*) as total FROM enquiries WHERE status='new'`);
    const [[monthRevenue]] = await db.execute(
      `SELECT COALESCE(SUM(amount),0) as total FROM fee_transactions 
       WHERE MONTH(payment_date)=MONTH(CURDATE()) AND YEAR(payment_date)=YEAR(CURDATE()) AND status='paid'`
    );
    ...
    const [recentStudents] = await db.execute(
      `SELECT u.name, u.created_at, s.enrollment_no FROM users u JOIN students s ON u.id=s.user_id
       WHERE u.role='student' ORDER BY u.created_at DESC LIMIT 5`
    );
    const [recentNotices] = await db.execute(
      `SELECT title, priority, publish_date FROM notices WHERE is_active=1 ORDER BY created_at DESC LIMIT 5`
    );
    const [upcomingExams] = await db.execute(
      `SELECT e.title, e.exam_date, b.name as batch_name, s.name as subject_name
       ...
       WHERE e.exam_date >= CURDATE() AND e.is_active=1
       ORDER BY e.exam_date ASC LIMIT 5`
    );
    const [monthlyRevenue] = await db.execute(
      `SELECT DATE_FORMAT(payment_date, '%b %Y') as month, SUM(amount) as revenue
       FROM fee_transactions WHERE status='paid' AND payment_date >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
...
exports.createSchedule = async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      `INSERT INTO schedules (batch_id, subject_id, faculty_id, day_of_week, start_time, end_time, room)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [batch_id, subject_id, faculty_id, day_of_week, start_time, end_time, room]
    );
    res.status(201).json({ success: true, message: 'Schedule added', id: result.insertId });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
  ...
  try {
    ...
    const [byBatch] = await db.execute(
      `SELECT b.name, SUM(ft.amount) as total, COUNT(*) as txn_count
       ...
       JOIN batches b ON ft.batch_id = b.id
       WHERE ft.status='paid' AND ft.payment_date BETWEEN ? AND ?
       GROUP BY b.id ORDER BY total DESC`,
      [from_date, to_date]
    );
    const [byMode] = await db.execute(
      `SELECT payment_mode, SUM(amount) as total, COUNT(*) as count
       FROM fee_transactions WHERE status='paid' AND payment_date BETWEEN ? AND ?
    ...
    const [[totalRevenue]] = await db.execute(
      `SELECT COALESCE(SUM(amount), 0) as total FROM fee_transactions WHERE status='paid' AND payment_date BETWEEN ? AND ?`,
      [from_date, to_date]
    );
    ...
    res.json({
      success: true,
      data: {
        revenue_by_batch: byBatch,
        revenue_by_mode: byMode,
