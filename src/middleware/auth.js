...
const checkLicense = async (req, res, next) => {
  ...
  try {
    const [rows] = await db.execute(
      'SELECT status, end_date FROM licenses WHERE org_id = ?', [req.orgId]
    );
    if (!rows.length) return res.status(403).json({ success: false, message: 'No license found' });
    if (rows[0].status !== 'active')
      return res.status(403).json({ success: false, message: `License ${rows[0].status}. Contact support.` });
    if (new Date(rows[0].end_date) < new Date())
      return res.status(403).json({ success: false, message: 'License expired. Please renew.' });
    next();
  } catch (err) {
